...
class NationalApiController extends Controller
{
     ...
     */
    public function getNationalPosts($locale): CategoryParentApiResource
    {
        ...
        if ($category) {
            ...
            $nationalPosts = $this->getPosts($locale)
                ...
                ->limit(4)
                ->get();
            $posts = $posts->merge(collect($nationalPosts)->pluck('id'));
            // put in cache
            Cache::forever('national_category_posts_' . $locale, $posts->toArray());

            $category->posts = PostWithImageApiResource::collection($nationalPosts);
        }
    ...
    /**
     * @OA\Get (
     *     path="/api/{locale}/category/national/politics",
     ...
     * @return CategoryParentApiResource
     */
    public function getNationalPoliticsPosts($locale): CategoryParentApiResource
    {
        $politics = (int)config('config.category.national_politics');

        return $this->getNationalChildPosts($locale, $politics);
    }

    /**
     * @OA\Get (
     *     path="/api/{locale}/category/national/country",
     ...
     * @return CategoryParentApiResource
     */
    public function getNationalCountryPosts($locale): CategoryParentApiResource
    {
        $country = (int)config('config.category.national_country');

        return $this->getNationalChildPosts($locale, $country);
    }

    /**
     * @OA\Get (
     *     path="/api/{locale}/category/national/crime",
     ...
     * @return CategoryParentApiResource
     */
    public function getNationalCrimePosts($locale): CategoryParentApiResource
    {
        $crime = (int)config('config.category.national_crime');

        return $this->getNationalChildPosts($locale, $crime);
    }

    /**
     * @OA\Get (
     *     path="/api/{locale}/category/national/more",
    ...
    public function getCategoryMorePosts(Request $request, $locale): JsonResponse
    {
        ...
        $categoryId = (int)config('config.category.national');
        $politicsCategoryId = (int)config('config.category.national_politics');
        $countryCategoryId = (int)config('config.category.national_country');
        $crimCategoryId = (int)config('config.category.national_crime');
        $ids = Cache::get('national_category_posts_' . $locale) ?? array();

        $posts = $this->getPosts($locale)
            ->whereIn('category_id', [$categoryId,$politicsCategoryId,$countryCategoryId,$crimCategoryId])
            // ->where('category_id', '=', $categoryId)
            ->whereNotIn('id', $ids)
            ->paginate($limit);
     ...
     * @return CategoryParentApiResource
     */
    private function getNationalChildPosts($locale, $categoryId): CategoryParentApiResource
    {
        $category = Category::with('translations')
            ->where('id', '=', $categoryId)
            ->first();
