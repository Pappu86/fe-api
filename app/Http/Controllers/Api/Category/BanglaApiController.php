...
class BanglaApiController extends Controller
{
     ...
     */
    public function getBanglaPosts($locale): CategoryParentApiResource
    {
        ...
        if ($category) {
            ...
            // put in cache
            Cache::forever('bangla_category_posts_' . $locale, $posts->toArray());
    ...
    /**
     * @OA\Get (
     *     path="/api/{locale}/category/bangla/more",
    ...
    public function getCategoryMorePosts(Request $request, $locale): JsonResponse
    {
        ...
        $categoryId = (int)config('config.category.bangla');
        $ids = Cache::get('bangla_category_posts_' . $locale) ?? array();
