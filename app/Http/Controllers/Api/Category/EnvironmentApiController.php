...
class EnvironmentApiController extends Controller
{
    use OpEdResponse;

    /**
     * @OA\Get (
     *     path="/api/{locale}/category/environment/home",
     ...
     */
    public function getEnvironmentPosts($locale): CategoryParentApiResource
    {
        App::setLocale($locale);

        $categoryId = (int)config('config.category.environment');
    ...
    public function getCategoryMorePosts(Request $request, $locale): JsonResponse
    {
        ...
        $categoryId = (int)config('config.category.environment');
        $ids = Cache::get('environment_category_posts_' . $locale) ?? array();
