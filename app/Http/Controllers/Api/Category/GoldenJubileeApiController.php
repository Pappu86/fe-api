...
class GoldenJubileeApiController extends Controller
{
    use OpEdResponse;

    /**
     * @OA\Get (
     *     path="/api/{locale}/category/golden-jubilee-of-independence/home",
     ...
     *     tags={"GoldenJubileeOfIndependenceCategoryApi"},
     *     @OA\Server(
     *     url="/",
     ...
     */
    public function getGoldenJubileeOfIndependencePosts($locale): CategoryParentApiResource
    {
        App::setLocale($locale);

        $categoryId = (int)config('config.category.golden_jubilee_of_independence');
        ...
        if ($category) {
            ...
            // put in cache
            Cache::forever('golden_jubilee_of_independence_category_posts_' . $locale, $posts->toArray());
    ...
    /**
     * @OA\Get (
     *     path="/api/{locale}/category/golden-jubilee-of-independence/more",
    ...
    public function getCategoryMorePosts(Request $request, $locale): JsonResponse
    {
        ...
        $categoryId = (int)config('config.category.golden_jubilee_of_independence');
        $ids = Cache::get('golden_jubilee_of_independence_category_posts_' . $locale) ?? array();
